...
我们现在来分析Code2DAG的一个建图的问题，生成DAG图
我们以zhang8为例子，busy_wait_seconds这里在建图的时候，我们进行了特殊处理，将它看作了一个函数实例，可是当它内部继续调用其他函数的时候，我们生成的图就会乱，原因是，当一个函数被调用时，如果我们记录只记录这个函数名，那么它被多次调用的时候，就会多个箭头指向函数名，这也是为什么要有实例存在，我的解决办法就是函数名加一层嵌套，代表了谁条用了这个函数，再加上数字，保证每个实例都是唯一的，但是现在遇到了main调用busy，busy又调用内部的时间函数，我的一级嵌套名就失效了，因此，我现在要做多级嵌套，以解决多重调用，我依旧可以做出图，你是否理解我的需求
