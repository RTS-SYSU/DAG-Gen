#!/usr/bin/env python3
"""
zhang2 六种调度策略 trace（第二轮输出）。
方法见 实验文档/约定术语.md §调度 Trace 分析法；权重见 实验文档/权值调整实验.md 第一轮方案。
"""
import subprocess, os, json

BASE = "/home/firefly/Desktop/ScratchDAG/权重实验分析（第二轮）"
...
def parse_trace(output):
    ...
    for line in output.strip().split("\n"):
        ...
        if len(parts) == 4 and parts[0] not in ("TOTAL:",):
            ...
            return lines, float(line.split()[1])
    return lines, 0


def main():
    results = {}
    for algo in ["CFS", "FIFO", "zhao2020", "heft", "t_level", "wcet_first"]:
    ...
    outp = os.path.join(BASE, "zhang2_all_traces.json")
    with open(outp, "w") as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
    print("\nDone! Results saved to", outp)
