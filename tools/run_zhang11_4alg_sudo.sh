  ...
  cat > "$cfg" <<JSON
{
  "tasks": [
    {
      "baseline_c": "$ROOT_DIR/中间结果/$BASE_NAME/pipeline/instrument/level2/$RULE/$algo/source_original.c",
      "prio_c": "$ROOT_DIR/中间结果/$BASE_NAME/pipeline/instrument/level2/$RULE/$algo/source_instrumented.c",
...
python3 - <<'PY'
import glob, json, os
for algo in ["cpf", "lpf", "heft", "zhao2020"]:
