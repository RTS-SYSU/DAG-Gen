...
def default_resume_file(config_path: Path, *, tool_dir: Optional[Path] = None) -> Path:
    """返回该 config 对应的 resume 状态文件路径（保证可写）。"""
    config_path = Path(config_path).expanduser().resolve()
    if tool_dir is None:
        tool_dir = Path(__file__).parent.parent.resolve()  # utils -> runtime_compare
    config_dir = (tool_dir / "配置文件").resolve()

    try:
        # 如果配置文件就在工具配置目录内，直接相邻存放
        config_path.relative_to(config_dir)
        return config_path.with_suffix(".resume.json")
    except Exception:
        # 否则统一放到 配置文件/.resume/ 下，用绝对路径 hash 避免重名
        resume_dir = tool_dir / "配置文件" / ".resume"
...
def apply_resume_to_task(task: Any) -> Tuple[bool, Optional[str]]:
    """如果 task 对应的 key 已完成，则将 task 标记为 done 并返回 True。"""
    ...
    task.status = "done"
    task.phase = "done"
    task.message = entry.get("message") or "已完成（断点续跑跳过）"
    task.out_dir = Path(entry["out_dir"])
    # 尽量保持 task_id 不变，方便 WebUI 查看日志/目录
