...
/*
 * zhang8: 与 zhang2 源码结构逐字一致（保证 Blocks 仅产生 MU 段、Pipeline 全流程可跑通）。
 * C1~C15 为缩小后的权重（仍经 busy_wait_seconds * WORK_SCALE）； wall 时间随 WORK_SCALE 与机器变化。
 */
...
SEG_BEGIN("MU:worker_c2#001@120-122");
  busy_wait_seconds(C1);
SEG_END("MU:worker_c2#001@120-122");
  pthread_mutex_unlock(&mutex_01);
  sem_wait(&sem_02);
  pthread_mutex_lock(&mutex_02);
SEG_BEGIN("MU:worker_c2#002@123-126");
  busy_wait_seconds(C2);
SEG_END("MU:worker_c2#002@123-126");
  pthread_mutex_unlock(&mutex_02);
  sem_wait(&sem_03);
  pthread_mutex_lock(&mutex_03);
SEG_BEGIN("MU:worker_c2#003@127-130");
  busy_wait_seconds(C3);
SEG_END("MU:worker_c2#003@127-130");
...
SEG_BEGIN("MU:worker_c1#001@136-138");
  busy_wait_seconds(C4);
SEG_END("MU:worker_c1#001@136-138");
  pthread_mutex_unlock(&mutex_04);
  sem_wait(&sem_01);
  pthread_mutex_lock(&mutex_05);
SEG_BEGIN("MU:worker_c1#002@139-142");
  busy_wait_seconds(C5);
SEG_END("MU:worker_c1#002@139-142");
...
SEG_BEGIN("MU:worker_c0#001@148-150");
  busy_wait_seconds(C7);
SEG_END("MU:worker_c0#001@148-150");
