...
static void *worker_a(void *arg) {
SEG_BEGIN("SEG:worker_a#001@100-100");
  (void)arg;
SEG_END("SEG:worker_a#001@100-100");
  pthread_mutex_lock(&mutex_03);
SEG_BEGIN("MU:worker_a#002@101-103");
  busy_wait_seconds(C3);
SEG_END("MU:worker_a#002@101-103");
  pthread_mutex_unlock(&mutex_03);
  pthread_mutex_lock(&mutex_04);
SEG_BEGIN("MU:worker_a#003@104-106");
  busy_wait_seconds(C4);
SEG_END("MU:worker_a#003@104-106");
...
/* --- filler_1: C5 → C8 --- */
static void *filler_1(void *arg) {
SEG_BEGIN("SEG:filler_1#001@112-112");
  (void)arg;
SEG_END("SEG:filler_1#001@112-112");
  pthread_mutex_lock(&mutex_05);
SEG_BEGIN("MU:filler_1#002@113-115");
  busy_wait_seconds(C5);
SEG_END("MU:filler_1#002@113-115");
  pthread_mutex_unlock(&mutex_05);
  pthread_mutex_lock(&mutex_08);
SEG_BEGIN("MU:filler_1#003@116-118");
  busy_wait_seconds(C8);
SEG_END("MU:filler_1#003@116-118");
...
/* --- filler_2: C6 --- */
static void *filler_2(void *arg) {
SEG_BEGIN("SEG:filler_2#001@124-124");
  (void)arg;
SEG_END("SEG:filler_2#001@124-124");
  pthread_mutex_lock(&mutex_06);
SEG_BEGIN("MU:filler_2#002@125-127");
  busy_wait_seconds(C6);
SEG_END("MU:filler_2#002@125-127");
...
/* --- filler_3: C7 --- */
static void *filler_3(void *arg) {
SEG_BEGIN("SEG:filler_3#001@133-133");
  (void)arg;
SEG_END("SEG:filler_3#001@133-133");
  pthread_mutex_lock(&mutex_07);
SEG_BEGIN("MU:filler_3#002@134-136");
  busy_wait_seconds(C7);
SEG_END("MU:filler_3#002@134-136");
...
/* --- main --- */
int main(void) {
SEG_BEGIN("SEG:main#001@142-144");
  init_matrices();

  /* main#001: 极轻 */
SEG_END("SEG:main#001@142-144");
  pthread_mutex_lock(&mutex_01);
SEG_BEGIN("MU:main#002@145-147");
  busy_wait_seconds(C1);
SEG_END("MU:main#002@145-147");
  pthread_mutex_unlock(&mutex_01);
SEG_BEGIN("SEG:main#003@148-153");
...
SEG_END("SEG:main#003@148-153");
SEG_BEGIN("SEG:main#004@154-155");

  /* join: 先 worker_a(关键路径), 再 filler */
SEG_END("SEG:main#004@154-155");
SEG_BEGIN("SEG:main#005@156-161");
  ...
  /* main#002: 极轻, 收尾 */
SEG_END("SEG:main#005@156-161");
  pthread_mutex_lock(&mutex_02);
SEG_BEGIN("MU:main#006@162-164");
  busy_wait_seconds(C2);
SEG_END("MU:main#006@162-164");
  pthread_mutex_unlock(&mutex_02);
SEG_BEGIN("SEG:main#007@165-165");

SEG_END("SEG:main#007@165-165");
  return 0;
}
