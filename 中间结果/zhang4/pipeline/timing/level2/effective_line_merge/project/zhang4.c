...
SEG_BEGIN("SEG:fill_worker_1#001@78-78");
  (void)arg;
SEG_END("SEG:fill_worker_1#001@78-78");
SEG_BEGIN("MU:fill_worker_1#002@79-81");
  pthread_mutex_lock(&mu_fill_1);
  busy_wait_seconds(F1);
  pthread_mutex_unlock(&mu_fill_1);
SEG_END("MU:fill_worker_1#002@79-81");
  return NULL;
}

static void *fill_worker_2(void *arg) {
SEG_BEGIN("SEG:fill_worker_2#001@86-86");
  (void)arg;
SEG_END("SEG:fill_worker_2#001@86-86");
SEG_BEGIN("MU:fill_worker_2#002@87-89");
  pthread_mutex_lock(&mu_fill_2);
  busy_wait_seconds(F2);
  pthread_mutex_unlock(&mu_fill_2);
SEG_END("MU:fill_worker_2#002@87-89");
  return NULL;
}

static void *critical_worker_1(void *arg) {
SEG_BEGIN("SEG:critical_worker_1#001@94-94");
  (void)arg;
SEG_END("SEG:critical_worker_1#001@94-94");
SEG_BEGIN("MU:critical_worker_1#002@95-98");
  pthread_mutex_lock(&mu_crit_1);
  busy_wait_seconds(C1);
  pthread_mutex_unlock(&mu_crit_1);
  sem_post(&sem_c12);
SEG_END("MU:critical_worker_1#002@95-98");
  return NULL;
}

static void *critical_worker_2(void *arg) {
SEG_BEGIN("SEG:critical_worker_2#001@103-103");
  (void)arg;
SEG_END("SEG:critical_worker_2#001@103-103");
SEG_BEGIN("MU:critical_worker_2#002@104-108");
  sem_wait(&sem_c12);
  pthread_mutex_lock(&mu_crit_2);
  busy_wait_seconds(C2);
  pthread_mutex_unlock(&mu_crit_2);
  sem_post(&sem_c23);
SEG_END("MU:critical_worker_2#002@104-108");
  return NULL;
}

static void *critical_worker_3(void *arg) {
SEG_BEGIN("SEG:critical_worker_3#001@113-113");
  (void)arg;
SEG_END("SEG:critical_worker_3#001@113-113");
SEG_BEGIN("MU:critical_worker_3#002@114-117");
  sem_wait(&sem_c23);
  pthread_mutex_lock(&mu_crit_3);
  busy_wait_seconds(C3);
  pthread_mutex_unlock(&mu_crit_3);
SEG_END("MU:critical_worker_3#002@114-117");
  return NULL;
}

int main(void) {
SEG_BEGIN("SEG:main#001@122-137");
  cpu_set_t cpu_set;
  CPU_ZERO(&cpu_set);
  CPU_SET(0, &cpu_set);
  CPU_SET(1, &cpu_set);
...
SEG_END("SEG:main#001@122-137");
SEG_BEGIN("SEG:main#002@138-138");
  pthread_create(&th_fill_2, NULL, fill_worker_2, NULL);
SEG_END("SEG:main#002@138-138");
SEG_BEGIN("SEG:main#003@139-139");
  pthread_create(&th_crit_1, NULL, critical_worker_1, NULL);
SEG_END("SEG:main#003@139-139");
SEG_BEGIN("SEG:main#004@140-140");
  pthread_create(&th_crit_2, NULL, critical_worker_2, NULL);
SEG_END("SEG:main#004@140-140");
SEG_BEGIN("SEG:main#005@141-141");
  pthread_create(&th_crit_3, NULL, critical_worker_3, NULL);
SEG_END("SEG:main#005@141-141");
SEG_BEGIN("SEG:main#006@142-142");

SEG_END("SEG:main#006@142-142");
SEG_BEGIN("SEG:main#007@143-143");
  pthread_join(th_fill_1, NULL);
SEG_END("SEG:main#007@143-143");
SEG_BEGIN("SEG:main#008@144-144");
  pthread_join(th_fill_2, NULL);
SEG_END("SEG:main#008@144-144");
SEG_BEGIN("SEG:main#009@145-145");
  pthread_join(th_crit_1, NULL);
SEG_END("SEG:main#009@145-145");
SEG_BEGIN("SEG:main#010@146-146");
  pthread_join(th_crit_2, NULL);
SEG_END("SEG:main#010@146-146");
SEG_BEGIN("MU:main#011@147-151");
  pthread_join(th_crit_3, NULL);

  pthread_mutex_lock(&mu_tail);
  busy_wait_seconds(TAIL);
  pthread_mutex_unlock(&mu_tail);
SEG_END("MU:main#011@147-151");
SEG_BEGIN("SEG:main#012@152-154");

  sem_destroy(&sem_c12);
  sem_destroy(&sem_c23);
SEG_END("SEG:main#012@152-154");
  return 0;
}
